...
// Book read-aloud audio is no longer generated on demand: admins upload a real narration
// MP3 for a paragraph (transcribed via speech-to-text — see transcribeBookAudio in
// admin.functions.ts) and it's saved straight onto that paragraph's audio_path /
// audio_word_timings / audio_text_hash fields, which the reader page reads directly off
// `getBook`'s response and plays from the public "book-audio" bucket. No separate
// server function is needed here for playback.
